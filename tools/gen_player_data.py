...
if K.EP == 6:
    ...
    # Map Keen (overworld): 4 cardinals x (walk, stand). Walk2 reuses walk1
    # in the pointer table (WRAM tight). Diagonals alias to cardinals at runtime.
    # FLAG: static planted flag (EGA sprite chunk 305).
    MAPKEEN_FRAMES = {
        "MK_N_W1": 260, "MK_N_ST": 262,
        "MK_E_W1": 257, "MK_E_ST": 259,
        "MK_S_W1": 263, "MK_S_ST": 265,
        "MK_W_W1": 254, "MK_W_ST": 256,
        "FLAG": 305,
    }
...
def main():
    ...
    # Pointer directories (ms_frames / enemy ms_*) live in bank 26 next to
    # player_draw/actors_draw — frees ~170B of the contested fixed region so
    # the map+ship+L1..L3 demo can link. font_tile is HUD-only -> bank 6.
    lines = ["// generated by gen_player_data.py\n#include \"player.h\"\n"]
    MS_RO = '__attribute__((section(".prg_rom_26.rodata"))) '
    HUD_RO = '__attribute__((section(".prg_rom_6.rodata"))) '
    names = []
    ...
    for name, fm in frames_meta.items():
        # Map Keen / flag: per-direction art, no hardware X-flip L slots.
        flips = (0,) if (name.startswith("MK_") or name == "FLAG") else (0, 1)
        for flip in flips:
    ...
    # frame lookup tables [anim][dir] — bank 26 (draw bank)
    lines.append(f"{MS_RO}const unsigned char *const ms_frames[][2] = {{\n")
    ...
    for o in order:
        lines.append(f"  {{ ms_{o}_R, ms_{o}_L }},\n")
    lines.append("};\n")
    # dummy stub so the OTHER episode's tables always link (bank 26 with dirs)
    lines.append(f"{MS_RO}static const unsigned char ms_none[] = {{ 128 }};\n")
    if BLOOG_FRAMES:
        lines.append(f"{MS_RO}const unsigned char *const ms_bloog[][2] = {{\n")
        for o in ["BLOOGW1", "BLOOGW2", "BLOOGW1", "BLOOGW2"]:
            lines.append(f"  {{ ms_{o}_R, ms_{o}_L }},\n")
        lines.append("};\n")
    else:
        lines.append(f"{MS_RO}const unsigned char *const ms_bloog[4][2] = "
                     "{ {ms_none, ms_none}, {ms_none, ms_none}, "
                     "{ms_none, ms_none}, {ms_none, ms_none} };\n")
    lines.append(f"{MS_RO}const unsigned char *const ms_bloogstun = "
    ...
    # a link-time constant. (gen_mmc5_rom asserts font ids < 128.) Bank 6 =
    # HUD bank (hud_draw already runs mapped there).
    lines.append(f"{HUD_RO}const unsigned char font_tile[] = {{ "
                 + ", ".join(str(packed_oam(font_tiles[c])) for c in glyph_chars)
                 + " };\n")
    lines.append(f'{HUD_RO}const char font_chars[] = "' + glyph_chars + '";\n')
    lines.append(f"{MS_RO}const unsigned char *const ms_platform = ms_PLATFORM_R;\n")
    # MapKeen: 8 dirs x 3 frames (walk1, walk2, stand). Only R slots — art is
    # already drawn per compass direction (no X-flip).
    # ms_mapkeen is emitted by gen_mmc5_rom into bank 26 (remapped tiles).
    # Stub here so the header always has a declaration; L0 image overrides.
    lines.append(
        "/* ms_mapkeen: defined in gen/leveldata_mmc5.c (per-level pack) */\n")
    if BABOBBA_FRAMES:
        lines.append(f"{MS_RO}const unsigned char *const ms_bab[][2] = {{ {{ms_BAB1_R, ms_BAB1_L}}, {{ms_BAB2_R, ms_BAB2_L}}, {{ms_BAB3_R, ms_BAB3_L}} }};\n")
    else:
        lines.append(f"{MS_RO}const unsigned char *const ms_bab[3][2] = "
    ...
    for pal, tag in (((3, "red"), (2, "grn")) if BLOOGLET_FRAMES else ()):
        ...
        lines.append(f"{MS_RO}const unsigned char *const ms_blet_{tag}[][2] = {{\n")
    ...
    if not BLOOGLET_FRAMES:
        for tag in ("red", "grn"):
            lines.append(f"{MS_RO}const unsigned char *const ms_blet_{tag}[5][2] = "
                         "{ {ms_none, ms_none}, {ms_none, ms_none}, {ms_none, ms_none}, "
                         "{ms_none, ms_none}, {ms_none, ms_none} };\n")
        ...
        for n in fnames:
            ...
            for flip in (0, 1):
                ...
                lines.append(f"const unsigned char ms_{tag}_{n}_{'L' if flip else 'R'}[] "
                             f"= {{ {', '.join(entries)} }};\n")
        lines.append(f"{MS_RO}const unsigned char *const ms_{tag}[][2] = {{\n")
        for n in rows:
    ...
    def stub_table(tag, nrows):
        lines.append(f"{MS_RO}const unsigned char *const ms_{tag}[{nrows}][2] = {{ "
    ...
    if SLUG_FRAMES:  # rows 0-1 crawl cycle (x2 to match ms_bloog indexing)
        ...
        lines.append(f"{MS_RO}const unsigned char *const ms_slugstun = ms_slug_SLUGSTUN_R;\n")
    else:
        slug_wh = (1, 1)
        stub_table("slug", 4)
        lines.append(f"{MS_RO}const unsigned char *const ms_slugstun = ms_none;\n")
    if MUSH_FRAMES:
    ...
    if SKYPEST_FRAMES:  # rows 0-1 fly cycle; SKYSQUASH = squished harmless
        ...
        lines.append(f"{MS_RO}const unsigned char *const ms_skypestsquash = "
                     "ms_skypest_SKYSQUASH_R;\n")
    else:
        skypest_wh = (1, 1)
        stub_table("skypest", 2)
        lines.append(f"{MS_RO}const unsigned char *const ms_skypestsquash = ms_none;\n")
    if BOUNDER_FRAMES:  # rows 0-1 bounce cycle (reused for horizontal); stun
        ...
        lines.append(f"{MS_RO}const unsigned char *const ms_bounderstun = "
                     "ms_bounder_BNDSTUN_R;\n")
    else:
        bounder_wh = (1, 1)
        stub_table("bounder", 2)
        lines.append(f"{MS_RO}const unsigned char *const ms_bounderstun = ms_none;\n")
    ...
    if WORM_FRAMES:
        ...
        lines.append(f"{MS_RO}const unsigned char *const ms_wormstun = "
                     "ms_worm_WORMSTUN_R;\n")
    else:
        worm_wh = (1, 1)
        stub_table("worm", 2)
        lines.append(f"{MS_RO}const unsigned char *const ms_wormstun = ms_none;\n")
    ...
    if MIM_FRAMES:
        ...
        lines.append(f"{MS_RO}const unsigned char *const ms_mimstun = "
                     "ms_mim_MIMSTUN_R;\n")
    else:
        mim_wh = (1, 1)
        stub_table("mim", 4)
        lines.append(f"{MS_RO}const unsigned char *const ms_mimstun = ms_none;\n")
    ...
    if SPARKY_FRAMES:  # 2-frame walk ping-pong in a 4-row walker table
        ...
        lines.append(f"{MS_RO}const unsigned char *const ms_sparkystun = "
                     "ms_sparky_SPARKYSTUN_R;\n")
    else:
        sparky_wh = (1, 1)
        stub_table("sparky", 4)
        lines.append(f"{MS_RO}const unsigned char *const ms_sparkystun = ms_none;\n")
    if AMPTON_FRAMES:
        ampton_wh = emit_enemy("ampton",
                               ["AMPTON1", "AMPTON2", "AMPTON1", "AMPTON2"],
                               box_names=["AMPTON1", "AMPTON2", "AMPTONSTUN"])
        lines.append(f"{MS_RO}const unsigned char *const ms_amptonstun = "
                     "ms_ampton_AMPTONSTUN_R;\n")
    else:
        ampton_wh = (1, 1)
        stub_table("ampton", 4)
        lines.append(f"{MS_RO}const unsigned char *const ms_amptonstun = ms_none;\n")
    if SLICE_FRAMES:
        slice_wh = emit_enemy("slice", ["SLICE"])
    else:
        slice_wh = (1, 1)
        stub_table("slice", 1)
    lines.append(f"{MS_RO}const unsigned char *const ms_shot[4] = "
    ...
    for ln in lines:
        ...
        if nm.startswith(("ms_HANG_", "ms_PULL")):
            ...
            continue
        if nm.startswith("ms_MK_") or nm.startswith("ms_FLAG"):
            # Map Keen / flag: banked only (gen_mmc5_rom remaps per-level).
            slot_layout[nm] = (-1, len(vals))
            continue
    ...
    ENEMY_FRAMESETS = {
        ...
        "slice": SLICE_FRAMES, "mapkeen": MAPKEEN_FRAMES,
    }
    ...
    manifest = {
        ...
        # REAL pose slots (ids may exceed 255) + map Keen/flag (banked-only).
        # gen_mmc5_rom remaps both into per-level images / bank 26.
        "ms_real": {nm: ms_real[nm] for nm in ms_real
                    if any(nm == f"ms_{fn}_{s}"
                           for fn in POSE_FRAMES for s in ("R", "L"))
                    or nm.startswith("ms_MK_")
                    or nm.startswith("ms_FLAG")},
    ...
    hdr.append("extern const unsigned char *const ms_mapkeen[8][3];\n")
    hdr.append("extern const unsigned char *const ms_mapflag;\n")
    # MapKeen is ~16x16; clip box in units (16 units/px) for walk probes.
    if MAPKEEN_FRAMES and "MK_E_ST" in frames_meta:
        mkw = frames_meta["MK_E_ST"]["width"]
        mkh = frames_meta["MK_E_ST"]["height"]
    else:
        mkw, mkh = 16, 16
    hdr.append(f"#define MAPKEEN_W {mkw}\n#define MAPKEEN_H {mkh}\n")
    hdr.append(f"#define MAPKEEN_W_U ({mkw} * 16)\n")
    hdr.append(f"#define MAPKEEN_H_U ({mkh} * 16)\n")
